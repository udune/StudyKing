namespace Gpm.LogViewer.Internal
{
    ...
    public class ConsoleView : ViewBase
    {
        ...
        public void SaveLog()
        {
            ...
            Log.Instance.SaveLogFile((exception, fileName) =>
            {
                WaitUi.Instance.ShowUi(false);
                
                if (exception != null)
                {
                    Debug.LogError(ConsoleViewConst.SAVE_FILE_FAILED);
                    Debug.LogException(exception);
                    Popup.Instance.ShowPopup(ConsoleViewConst.SAVE_FILE_FAILED, Common.ERROR);

                }
                else
                {
                    string message = string.Format(ConsoleViewConst.SAVE_FILE_SUCCEEDED, fileName);
                    Debug.Log(message);
                    Popup.Instance.ShowPopup(message);
                }
