namespace Gpm.LogViewer.Internal
{
    ...
    public class Log : MonoBehaviour
    {
        ...
        public void SaveLogFile(Action<Exception, string> callback)
        {
            string logFilePath = Path.Combine(Application.persistentDataPath, string.Format(LogConst.LOGFILE_NAME, DateTime.Now.Ticks));
            WriteFullLogToFile(logFilePath, exception =>
            {
                if (exception != null)
                {
                    callback(exception, "");
        ...
        {
            string logFilePath = Path.Combine(Application.persistentDataPath, LogConst.LOGFILE_MAIL_NAME);
            WriteFullLogToFile(logFilePath, exception =>
            {
                if (exception != null)
                {
        ...
        public void WriteFullLogToFile(string filePath, Action<Exception> callback)
        {
            StartCoroutine(GetFullLogInformation(logInformation =>
            {
                try
                {
                    using (var stream = new FileStream(filePath, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite | FileShare.Delete))
                    {
